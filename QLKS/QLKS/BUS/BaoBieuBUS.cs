...
namespace QLKS.BUS
{
    class BaoBieuBUS
    {
        ...
        public DataTable LayDuDieuLocTheoNgayThang(DateTime dtBD, DateTime dtKT)
        {
            DataTable table = new DataTable();
            ...
            foreach (var item in _data)
            {
                ...
                if (DateTime.TryParse(item.ThoiGianBatDau,out ThoiGianBatDau) 
                    && DateTime.TryParse(item.ThoiGianBatDau, out ThoiGianKetThuc))
                {
                    TimeSpan spanBD = ThoiGianBatDau - dtBD;
                    TimeSpan spanKT = dtKT - ThoiGianKetThuc;
                    if (spanBD.TotalSeconds > 0 && spanKT.TotalSeconds > 0)
                    {
                        string s = Global.Instance().ConvertCurrency(item.TongTien.ToString());
                        table.Rows.Add(item.IdBaoBieu.ToString(), item.IdPhong.ToString(), item.IdNhanVien.ToString(), item.IdKhachHang.ToString(), item.ThoiGianBatDau.ToString(), item.ThoiGianKetThuc.ToString(), item.DsDichVu.ToString(), s.ToString());
                    }
